...
public class SensorLib {
    ...
    public static class PID {
        ...
        private float mKiMaxSum = 0;   // max integral to prevent windup
        private float mMinOutput = -1.0f;
        private float mMaxOutput = 1.0f;
        private boolean mOutputBounded = false;

        public PID(float Kp, float Ki, float Kd, float KiCutoff) {
            setK(Kp, Ki, Kd, KiCutoff);
        }

        public PID(float Kp, float Ki, float Kd, float KiCutoff, float KiMaxSum, float minOutput, float maxOutput) {
            setK(Kp, Ki, Kd, KiCutoff);
            setMaxSum(KiMaxSum);
            setOutputBounds(minOutput, maxOutput);
        }
        ...
        public void setOutputBounds(float min, float max) {
            mMinOutput = min;
            mMaxOutput = max;
            mOutputBounded = true;
        }
        ...
        public float loop(float error, float dt) {
            if (Math.abs(error) < mKiCutoff) {     // only integrate some errors
                mIntegral += (error + mPrevError) * dt / 2.0;

                if (mKiMaxSum != 0 && Math.abs(mIntegral) > mKiMaxSum) {
                    mIntegral = Math.signum(mIntegral) * Math.abs(mKiMaxSum);
                }
            }
            ...
            if (mOutputBounded) {
                output = Range.clip(output, mMinOutput, mMaxOutput);
            }
